...
from app.config import (
    EMPLOYMENT_LOG_OBJECT_API_NAME,     # Use this as the key for Apex call
    EMPLOYMENT_DETAIL_OBJECT_API_NAME,  # Use this for contextual logging
...
async def process_single_employment_detail(
    ...
    detail_sobject_context_name = EMPLOYMENT_DETAIL_OBJECT_API_NAME # For logging and clarity

    logger.info(f"Background task started for Employment Log (via {record_sobject_api_name_key_for_apex}) ID: {employment_log_id} "
    ...
    actual_employment_detail_id: Optional[str] = None # Renamed for clarity (this will be Affiliation ID)
    ...
    try:
        ...
        if not details:
            ...
            if not record_data:
                logger.warning(f"No 'recordData' (expected {detail_sobject_context_name} details) found for Employment Log ID: {employment_log_id}.")
                ...
                if not actual_employment_detail_id:
                    logger.error(f"CRITICAL: {detail_sobject_context_name} ID (Affiliation ID) not found in recordData for Employment Log {employment_log_id}. "
                                 f"{APPLICATION_VERIFICATION_SUMMARY_OBJECT_API_NAME} record cannot be reliably linked.")
                    final_report_to_salesforce = f"Error: Could not identify underlying {detail_sobject_context_name} ID from data linked to Log {employment_log_id}."
                    ...
                    logger.info(f"Processing documentPayload '{file_name}' for Employment Log ID: {employment_log_id} (Actual {detail_sobject_context_name} ID: {actual_employment_detail_id})")
                    ...
                    if base64_data and file_extension:
                        try:
                            ...
                            if document_text_string.startswith("Error:"):
                                ...
                                overall_status_log_message = "Completed - Employment document verification by crew." if "Error:" not in final_report_to_salesforce else "Completed - Employment crew reported an error."
                        ...
                        final_report_to_salesforce = "Verification Info (Employment): Document data missing in payload. Cannot verify."
                        overall_status_log_message = "Completed - No document data in payload."
                elif actual_employment_detail_id: # ID was found, but no document payload
            ...
            if record_data: logger.debug(f"RecordData ({detail_sobject_context_name}) for Log ID {employment_log_id}: {json.dumps(record_data, indent=2)}")
        ...
        if actual_employment_detail_id:
            success_update = sf_service.upsert_verification_summary(
                ...
                affiliation_id=actual_employment_detail_id # This is the Affiliation ID
            )
        else: logger.error(f"Cannot update AVS for Employment Log {employment_log_id} because {detail_sobject_context_name} ID is missing.")

        if success_update: logger.info(f"Successfully updated AVS for Employment (Detail ID: {actual_employment_detail_id}). Status: {overall_status_log_message}")
        elif actual_employment_detail_id: logger.error(f"Failed to update AVS for Employment (Detail ID: {actual_employment_detail_id}). Status: {overall_status_log_message}")

    except Exception as e:
        logger.error(f"Unexpected error for Employment Log ID {employment_log_id}: {e}", exc_info=True)
        error_report_content = f"Critical Error for Employment Log ID {employment_log_id}:\n{str(e)[:1000]}"
        retrieved_detail_id = locals().get('actual_employment_detail_id')
        summary_name_id_part_on_error = retrieved_detail_id or employment_log_id
        summary_name_on_error = f"Employment History Analysis - CRITICAL ERROR ({item_index})" if item_index is not None else f"Employment History Analysis - CRITICAL ERROR - {summary_name_id_part_on_error}"
        try:
            if locals().get('sf_service') and retrieved_detail_id:
                locals()['sf_service'].upsert_verification_summary(parent_application_id, error_report_content, summary_name_on_error, affiliation_id=retrieved_detail_id)
            else: logger.error(f"Cannot update AVS with critical error for Employment Log {employment_log_id}: sf_service or detail ID missing.")
        except Exception as update_err: logger.error(f"Failed to update AVS with critical error for Employment Log {employment_log_id}: {update_err}", exc_info=True)