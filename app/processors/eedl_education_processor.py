...
_READABLE = "Education Verification"
...
async def process_eedl_education_record(
    ...
    logger.info(f"Starting {_READABLE} processing for Education ID: {education_id}")
    ...
    record_type_label = f"Education_{item_index or education_id}"

    try:
        details = await asyncio.to_thread(
            sf_service.get_eedl_education_record_data, education_id, parent_opportunity_id
        )
        ...
        if skip:
            logger.info(f"Skipping {_READABLE} {education_id}: {reason}")
        ...
        # Fallback: no document or SF data issue
        if salesforce_data_issue or not document_payload:
            fallback_msg = salesforce_data_issue or f"No matching document found for Education record {education_id}."
            logger.warning(f"{_READABLE} fallback for {education_id}: {fallback_msg}")
            await asyncio.to_thread(
                sf_service.upsert_eedl_verification_summary,
                parent_opportunity_id,
                EEDL_VS_RECORD_TYPE_EDUCATION,
                f"Education Analysis ({degree_label})",
        ...
        if not base64_data or not file_extension:
            raise ValueError("Document content (base64) or file extension missing from education document payload.")
        ...
        reset_global_usage()
        from app.langgraph.eedl_education_graph import EEDLEducationGraphOrchestrator
        orchestrator = EEDLEducationGraphOrchestrator(record_data, document_text)
        report_dict = await asyncio.to_thread(orchestrator.run)

        if not report_dict:
            raise ValueError("EEDL Education graph did not return a valid report.")

        crew_usage = _capture_usage()
        logger.info(f"Graph processing usage for {education_id}: {crew_usage}")
        ...
        logger.info(f"Successfully processed {_READABLE} for Education {education_id}.")
        return f"Successfully processed {_READABLE} ({degree_label})."
        ...
        raise ValueError(f"(Education)-({education_id})-(Salesforce API)-(An API error occurred): {e}")
        ...
        raise ValueError(f"(Education)-({education_id})-(Document Extraction)-(Failed to extract text): {e}")
        ...
        raise ValueError(f"(Education)-({education_id})-(Processing)-(An unexpected error occurred): {e}")
